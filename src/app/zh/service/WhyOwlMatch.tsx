...
/**
 * 「WHY OWLMATCH」説得セクション — zh版
 * ja版（src/app/service/WhyOwlMatch.tsx）と構造・スタイルは完全に同一。文字列のみ簡体字中国語に差し替え。
 */
...
const WALLS = [
  { name: "语言的壁垒", problem: "「会说」，含糊不清。", x: "50%", y: "16%" },
  { name: "实力的壁垒", problem: "简历上，看不出来。", x: "79%", y: "67%" },
  { name: "匹配偏差", problem: "凭感觉选，摇摆不定。", x: "21%", y: "67%" },
];

const ROWS: { axis: string; old: string; owl: string }[] = [
  { axis: "费用・报酬", old: "先收取定金・介绍费，退款流程也复杂", owl: "完全成功报酬制。录用确定之前，费用为零。" },
  { axis: "语言・实务", old: "含糊的自我申报", owl: "以日语 N2 以上・中文实务能力为标准。" },
  { axis: "匹配方式", old: "依赖负责人的经验直觉", owl: "由 AI 比对意愿与技能。" },
  { axis: "法务・合同・在留", old: "全权交由企业自行处理", owl: "作为持牌机构，合同与在留资格一并设计。" },
  { axis: "速度", old: "数周至数月", owl: "需求确定后，最短时间即可见面。" },
  { axis: "中国人才库", old: "难以触达", owl: "直接连接即战力人才库。" },
];
    ...
    <section className="relative overflow-hidden bg-[#FAFAF7] pt-40 pb-28 md:pt-36 md:pb-36 px-6" aria-labelledby="why-heading">
      ...
      <div className="container mx-auto max-w-7xl">
        ...
        <h2 id="why-heading" className="text-[38px] md:text-[58px] font-medium tracking-[-0.03em] text-slate-900 leading-[1.08]">
          跨境招聘的壁垒，<span className="inline-block">靠机制打破。</span>
        </h2>
          ...
          <div ref={diagramRef} className="relative mx-auto aspect-square w-full max-w-[720px]">
            ...
            <div className="wd-core absolute left-1/2 top-1/2 z-20 grid h-[170px] w-[170px] max-[420px]:h-[148px] max-[420px]:w-[148px] md:h-[200px] md:w-[200px] place-items-center rounded-full border border-cyan-200 bg-white text-center shadow-[0_18px_40px_-18px_rgba(6,182,212,0.45)]">
              <div>
                ...
                <span className="mt-1 block font-mono text-[10px] md:text-[11px] uppercase tracking-[0.18em] text-cyan-700">突破</span>
              </div>
            </div>
          ...
          <div className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white">
            <div className="grid grid-cols-[1fr_1fr] md:grid-cols-[180px_1fr_1fr]">
              ...
              <div className="border-b border-r border-slate-200 px-5 md:px-6 py-4">
                <span className="font-mono text-[10px] uppercase tracking-[0.18em] text-slate-400">传统人才中介</span>
              </div>
